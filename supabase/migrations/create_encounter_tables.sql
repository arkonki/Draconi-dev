/*
      ...
      1. New Tables
        ...
        - `encounter_combatants`
          ...
          - `is_player_character` (boolean, not null): True if the combatant is a player character.
      ...
      5. Security
        ...
        - Policies:
          ...
          - Authenticated users (party members) can read encounter and combatant data for their party's encounters.
    */
    ...
    CREATE POLICY "Party members can view their encounters"
      ON public.encounters
      FOR SELECT
      USING (
        EXISTS (
          SELECT 1 FROM public.party_members pm
          WHERE pm.party_id = encounters.party_id AND pm.user_id = auth.uid()
        )
        OR
