...
import {
  Shield, Heart, HelpCircle, Swords, Brain, Zap, Users, Moon, Sun, Clock, Skull, Package, Book, GraduationCap, Star, Sparkles, X, Bed, Award, ShieldCheck, HeartPulse, UserCog, Dumbbell, Feather, UserSquare, StickyNote, Plus, Save, Trash2
} from 'lucide-react';
...
// --- NEW: Dedicated component for Character-Specific Notes ---
    ...
    <div className="bg-white p-4 rounded-lg border">
      ...
      <div className="space-y-2">
        {notes.length === 0 ? (
          ...
          notes.map(note => (
            <div key={note.id} className="p-2 rounded-md hover:bg-gray-50 flex justify-between items-center">
              <button onClick={() => setEditingNote(note)} className="text-sm font-medium text-gray-800 hover:text-blue-600">{note.title}</button>
      ...
      {editingNote && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-2xl w-full p-6 shadow-xl flex flex-col max-h-[90vh]">
            ...
            <div className="overflow-y-auto space-y-4 flex-grow">
              <input type="text" placeholder="Note Title" value={editingNote.title || ''} onChange={e => setEditingNote(prev => ({ ...prev, title: e.target.value }))} className="w-full font-bold text-lg p-2 border rounded" />
              <textarea placeholder="Note Content (Markdown supported)..." value={editingNote.content || ''} onChange={e => setEditingNote(prev => ({...prev, content: e.target.value}))} rows={15} className="w-full p-2 border rounded font-mono text-sm" />
            </div>
...
export function CharacterSheet({}: CharacterSheetProps) {
  ...
  const {
    character, fetchCharacter, adjustStat, toggleCondition, performRest, isLoading, error, isSaving, saveError,
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-lg max-w-lg w-full p-6 max-h-[90vh] overflow-y-auto shadow-xl">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-xl font-semibold flex items-center gap-2"><Bed className="w-6 h-6" /> Choose Rest Type</h3>
            <button onClick={() => { setShowRestOptionsModal(false); setHealerPresent(false); }} className="text-gray-500 hover:text-gray-700"><X className="w-6 h-6" /></button>
          </div>
          <div className="space-y-4 mb-6">
            <div><button onClick={() => handleRest('shift')} className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50" disabled={isSaving}>{isSaving ? 'Resting...' : 'Take Shift Rest'}</button><p className="text-sm text-gray-600 mt-2"><strong>Shift Rest (~6 hours):</strong> Requires a safe location. Recovers all HP & WP, heals all standard conditions.</p></div>
            <div><button onClick={() => handleRest('stretch')} className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 disabled:opacity-50" disabled={isSaving || (character?.current_hp ?? 0) <= 0}>{isSaving ? 'Resting...' : `Take Stretch Rest`}</button><p className="text-sm text-gray-600 mt-2"><strong>Stretch Rest (~15 mins):</strong> Heal {healerPresent ? '2d6' : '1d6'} HP, recover 1d6 WP, heal one condition. Cannot be taken while dying.</p></div>
            <div className="mb-4"><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={healerPresent} onChange={(e) => setHealerPresent(e.target.checked)} className="rounded border-gray-300" /><span>Healer present for Stretch Rest</span></label></div>
            <div><button onClick={() => handleRest('round')} className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50" disabled={isSaving}>{isSaving ? 'Resting...' : 'Take Round Rest'}</button><p className="text-sm text-gray-600 mt-2"><strong>Round Rest (~10 secs):</strong> Recover 1d6 WP. No HP recovery.</p></div>
          </div>
    ...
    <div className="p-4 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
        ...
        <div className="md:col-span-6 grid grid-cols-2 gap-4">
          {currentHP > 0 ? ( <div className="p-3 rounded-lg shadow bg-white"><div className="flex items-center justify-between mb-1"><h3 className="font-semibold flex items-center gap-1 text-sm"><HeartPulse className="w-4 h-4 text-red-600" /> HP</h3><span className="text-sm font-medium">{currentHP} / {maxHP}</span></div><div className="flex gap-2 mt-1"><button onClick={() => adjustStat('current_hp', -1)} className="flex-1 px-2 py-1 text-xs bg-red-100 text-red-700 rounded hover:bg-red-200 disabled:opacity-50" disabled={isSaving || currentHP <= 0}>-</button><button onClick={() => adjustStat('current_hp', 1)} className="flex-1 px-2 py-1 text-xs bg-green-100 text-green-700 rounded hover:bg-green-200 disabled:opacity-50" disabled={currentHP >= maxHP || isSaving}>+</button></div></div> ) : ( <DeathRollTracker character={character} /> )}
          <div className="p-3 bg-white rounded-lg shadow"><div className="flex items-center justify-between mb-1"><h3 className="font-semibold flex items-center gap-1 text-sm"><Zap className="w-4 h-4 text-blue-600" /> WP</h3><span className="text-sm font-medium">{currentWP} / {maxWP}</span></div><div className="flex gap-2 mt-1"><button onClick={() => adjustStat('current_wp', -1)} className="flex-1 px-2 py-1 text-xs bg-red-100 text-red-700 rounded hover:bg-red-200 disabled:opacity-50" disabled={currentWP === 0 || isSaving}>-</button><button onClick={() => adjustStat('current_wp', 1)} className="flex-1 px-2 py-1 text-xs bg-green-100 text-green-700 rounded hover:bg-green-200 disabled:opacity-50" disabled={currentWP >= maxWP || isSaving}>+</button></div></div>
        </div>
      </div>
