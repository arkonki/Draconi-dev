...
const baseSkillAttributeMap: Record<string, AttributeName> = {
...
export function CharacterCreationWizard() {
  ...
  const canProceed = () => {
    switch (step) {
      ...
      case 7: return !!character.appearance && character.appearance.trim().length > 0;
...
const handleSave = async () => {
  if (!user) { setError('You must be logged in to create a character'); return; }
  if (!canProceed()) { setError('Please complete all required fields for the final step before saving.'); return; }
  if (!character.attributes || !character.profession) { setError('Attributes or Profession are missing.'); return; }
  ...
  try {
    ...
    // Fetch the details of the selected profession directly from the database.
    console.log(`[CharacterSave] Fetching details for profession: ${character.profession}`);
    const { data: professionDetails, error: professionError } = await supabase
      .from('professions')
      .select('is_magic, associated_skill, key_attribute') // CHANGED
      .eq('name', character.profession)
      .single();

    if (professionError) {
      throw new Error(`Could not find details for profession '${character.profession}'. Please check the database.`);
    }
    if (!professionDetails) {
        throw new Error(`No data returned for profession '${character.profession}'.`);
    }

    const masterSkillMap: Record<string, AttributeName> = { ...baseSkillAttributeMap };
    
    // Dynamically add the profession's skill (if any) to the master map.
    if (professionDetails.is_magic && professionDetails.associated_skill && professionDetails.key_attribute) { // CHANGED
      masterSkillMap[professionDetails.associated_skill] = professionDetails.key_attribute as AttributeName; // CHANGED
      console.log(`[CharacterSave] Dynamically added '${professionDetails.associated_skill}' to skill map.`);
    }

    const allSkillsToProcess = new Set([
      ...Object.keys(baseSkillAttributeMap),
    ...
    allSkillsToProcess.forEach(skillName => {
      const attribute = masterSkillMap[skillName];
      if (!attribute) {
        console.warn(`[CharacterSave] Skill '${skillName}' has no attribute mapping and will be skipped.`);
        return;
      }
      if (!character.attributes || !(attribute in character.attributes)) {
          console.error(`[CharacterSave] Attribute '${attribute}' for skill '${skillName}' not found on character.`);
          return;
      }
      ...
      const isProfessionSkill = professionDetails.associated_skill === skillName;
      
      initialSkillLevels[skillName] = (isTrained || isProfessionSkill) ? baseChance * 2 : baseChance;
    });
    ...
    const characterData = {
      ...
      equipment: character.equipment || { inventory: [], equipped: { weapons: [] }, money: { gold: 0, silver: 0, copper: 0 } },
      appearance: character.appearance?.trim(),
      conditions: character.conditions || { exhausted: false, sickly: false, dazed: false, angry: false, scared: false, disheartened: false },
    ...
    if (!savedCharacter) throw new Error('Character not created or data not returned.');
    ...
    if (message.includes('unique constraint')) {
        setError(`Failed to save character: A character with this name might already exist.`);
    } else {
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-xl p-6 md:p-8 my-8">
            ...
            <div className="flex space-x-1">
                {steps.map((s, i) => (
                    <div key={s.title} className="flex-1 h-2 rounded-full overflow-hidden bg-gray-200">
                        <div
                            className={`h-full rounded-full transition-all duration-300 ${i < step ? 'bg-blue-500' : i === step ? 'bg-blue-300' : 'bg-gray-200'}`}
        ...
        <div className="flex justify-between items-center mt-8 border-t pt-6">
            <button onClick={() => setStep(step - 1)} disabled={step === 0 || saving} className="px-5 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed">
                Previous
            </button>
            {step === steps.length - 1 ? (
                ...
                <button onClick={() => setStep(step + 1)} disabled={!canProceed() || saving} className={`inline-flex items-center px-5 py-2 text-sm font-medium text-white rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-150 ${saving ? 'bg-gray-400 cursor-not-allowed' : !canProceed() ? 'bg-gray-300 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}>
                    Next Step
                </button>
            )}
        </div>
