...
const SpellDetailPane = ({ spell, onClose }: { spell: DetailedSpell | null; onClose: () => void; }) => { if (!spell) return null; const foundRequirements = Object.keys(requirementExplanations).filter(key => spell.requirement?.includes(key)); return ( <> <div className="fixed inset-0 bg-black bg-opacity-25 z-50" onClick={onClose} aria-hidden="true" /> <div className={`fixed top-0 right-0 h-full w-full max-w-md bg-white shadow-2xl transform transition-transform duration-300 ease-in-out z-50 ${spell ? 'translate-x-0' : 'translate-x-full'}`}> <div className="p-6 h-full flex flex-col"> <div className="flex justify-between items-center border-b pb-4"><h3 className="text-2xl font-bold text-gray-800">{spell.name}</h3><button onClick={onClose} className="text-gray-500 hover:text-gray-700"><X className="w-6 h-6" /></button></div> <div className="overflow-y-auto mt-4 flex-grow space-y-6 text-gray-700"> <div><h4 className="font-semibold text-gray-800 mb-1">Description</h4><p className="text-sm italic border-l-4 border-gray-200 pl-4 py-1">{spell.description}</p></div> {spell.requirement && (<div><h4 className="font-semibold text-gray-800 mb-2">Requirements</h4><div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800 space-y-2">{foundRequirements.length > 0 ? (foundRequirements.map(key => (<p key={key}><strong>{key}:</strong> {requirementExplanations[key]}</p>))) : (<p>{spell.requirement}</p>)}</div></div>)} <div><h4 className="font-semibold text-gray-800 mb-2">Properties</h4><div className="space-y-2 text-sm"><div className="flex justify-between border-b py-2"><span className="font-medium">Rank</span><span>{spell.rank === 0 ? 'Trick' : spell.rank}</span></div><div className="flex justify-between border-b py-2"><span className="font-medium">WP Cost</span><span>{spell.willpowerCost}</span></div><div className="border-b py-2"><div className="flex justify-between"><span className="font-medium">Casting Time</span><span>{spell.castingTime}</span></div></div><div className="border-b py-2"><div className="flex justify-between"><span className="font-medium">Range</span><span>{spell.range}</span></div></div><div className="border-b py-2"><div className="flex justify-between"><span className="font-medium">Duration</span><span>{spell.duration}</span></div>{durationExplanations[spell.duration] && (<blockquote className="mt-2 text-xs italic text-gray-500 border-l-4 pl-3">{durationExplanations[spell.duration]}</blockquote>)}</div></div></div></div></div></div></> ); };
...
export function SpellcastingView({ onClose }: SpellcastingViewProps) {
  ...
  // --- (handleTogglePrepare and other hooks are unchanged) ---
  ...
  const { actualMagicSkillName, magicSkillValue, isMagicSkillAffected } = useMemo(() => {
    ...
    // Directly find which magic school skill the character actually possesses.
    // This is more robust than relying on a separate schoolName property.
    let determinedSkillName = KNOWN_MAGIC_SKILLS.find(skill => characterSkills.has(skill));

    // If no specific school is found, check if they have General Magic
    if (!determinedSkillName && characterSkills.has('General Magic')) {
      determinedSkillName = 'General Magic';
    }
    ...
    // Fallback if no magic skill is found at all
    return { actualMagicSkillName: null, magicSkillValue: null, isMagicSkillAffected: false };
  }, [character]);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 border-b bg-white hover:bg-gray-50/50 last:border-b-0">
        <div className="flex-grow">
          <div className="flex items-center justify-between">
            <button onClick={() => setInfoPaneSpell(spell)} className="text-left"><h3 className="font-semibold text-gray-800 hover:text-blue-600 transition-colors">{spell.name}</h3></button>
            <span className={`text-xs font-medium px-2 py-1 rounded-full ${isTrick ? 'bg-gray-100 text-gray-700' : 'bg-blue-100 text-blue-800'}`}>{isTrick ? 'Trick' : `Rank ${spell.rank}`}</span>
          </div>
