      ...
      <div className={`fixed top-0 right-0 h-full w-full max-w-md bg-white shadow-2xl transform transition-transform duration-300 ease-in-out z-50 ${spell ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="p-6 h-full flex flex-col">
          <div className="flex justify-between items-center border-b pb-4">
            <h3 className="text-2xl font-bold text-gray-800">{spell.name}</h3>
...
const getBaseChance = (value: number): number => {
  if (value <= 5) return 3; if (value <= 8) return 4; if (value <= 12) return 5;
  if (value <= 15) return 6; return 7;
};
...
export function SpellcastingView({ onClose }: SpellcastingViewProps) {
  ...
  const { actualMagicSkillName, magicSkillValue, isMagicSkillAffected } = useMemo(() => {
    if (!character || !characterSchoolName) return { actualMagicSkillName: null, magicSkillValue: null, isMagicSkillAffected: false };
    const skillLevels = character.skill_levels || {};
    const trainedSkills = character.trainedSkills || [];
    const skillName = trainedSkills.includes(characterSchoolName) ? characterSchoolName : 'General Magic';
    const storedLevel = skillLevels[skillName];
    const finalSkillValue = storedLevel !== undefined ? storedLevel : calculateFallbackLevel(character, skillName);
    return { actualMagicSkillName: skillName, magicSkillValue: finalSkillValue, isMagicSkillAffected: character.conditions?.scared ?? false };
        ...
        <div className="flex-shrink-0 mt-4 sm:mt-0 sm:ml-4">
          {activeTab === 'grimoire' ? (
            <div className="flex items-center justify-end gap-2 w-48">
              {isPowerLevelSpell ? (
                <div className="flex items-center gap-1">
                  <Button variant="outline" size="xs" onClick={() => handleLevelChange(-1)} disabled={level <= 1 || !!castingSpellId || isSaving || isReactionSpell} className="px-2" title="Decrease level"><Minus size={14} /></Button>
                  <Button variant="secondary" size="xs" onClick={() => handleCastSpell(spell, level, true)} disabled={isReactionSpell || !!castingSpellId || isSaving || insufficientWp} className="flex-grow min-w-[70px]" title={isReactionSpell ? "Reaction spells must be prepared" : `Cast Lvl ${level} from grimoire (double time)`}>{castingSpellId === spell.id ? '' : `Cast Lvl ${level}`}</Button>
                  <Button variant="outline" size="xs" onClick={() => handleLevelChange(1)} disabled={level >= 3 || !!castingSpellId || isSaving || isReactionSpell} className="px-2" title="Increase level"><Plus size={14} /></Button>
                </div>
              ) : (
                <Button variant="secondary" size="xs" onClick={() => handleCastSpell(spell, 1, true)} disabled={isReactionSpell || !!castingSpellId || isSaving || insufficientWp} className="w-full" title={isReactionSpell ? "Reaction spells must be prepared" : "Cast from grimoire (double time)"}>Cast</Button>
              )}
              <button onClick={() => handleTogglePrepare(spell.id, isPrepared)} disabled={!isPrepared && !canPrepareMore || isSaving} className="disabled:opacity-50 disabled:cursor-not-allowed" title={isPrepared ? "Unprepare" : canPrepareMore ? "Prepare" : `Limit reached`}>
                {isPrepared ? <CheckSquare className="w-5 h-5 text-blue-600 hover:text-blue-800" /> : <Square className={`w-5 h-5 ${canPrepareMore ? 'text-gray-400 hover:text-gray-600' : 'text-gray-300'}`} />}
              </button>
            </div>
          ) : (
            <div className="flex items-center justify-end gap-1 w-40">
              {isPowerLevelSpell ? (
                <>
                  <Button variant="outline" size="xs" onClick={() => handleLevelChange(-1)} disabled={level <= 1 || !!castingSpellId || isSaving} className="px-2" title="Decrease level"><Minus size={14} /></Button>
                  <Button variant="primary" size="xs" onClick={() => handleCastSpell(spell, level, false)} loading={castingSpellId === spell.id || isSaving} disabled={!!castingSpellId || isSaving || insufficientWp} className="flex-grow min-w-[70px]" title={insufficientWp ? `Need ${actualWpCost} WP` : `Cast Lvl ${level}`}>{castingSpellId === spell.id ? '' : `Cast Lvl ${level}`}</Button>
                  <Button variant="outline" size="xs" onClick={() => handleLevelChange(1)} disabled={level >= 3 || !!castingSpellId || isSaving} className="px-2" title="Increase level"><Plus size={14} /></Button>
                </>
              ) : (
                <Button variant="primary" size="xs" onClick={() => handleCastSpell(spell, 1, false)} loading={castingSpellId === spell.id || isSaving} disabled={!!castingSpellId || isSaving || insufficientWp} className="w-full" title={insufficientWp ? `Need ${actualWpCost} WP` : `Cast ${spell.name}`}>{castingSpellId === spell.id ? '' : 'Cast'}</Button>
              )}
            </div>
          )}
        </div>
          ...
          <div className="p-4 border-b bg-gray-50">
            ...
            <div className="border-b border-gray-200">
              <nav className="-mb-px flex space-x-4">
                ...
                <div className="py-3 px-1 text-sm text-gray-500">Slots: <span className="font-medium">{preparedRankedSpellCount}/{preparationLimit}</span></div>
              </nav>
            </div>
