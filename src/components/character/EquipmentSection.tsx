...
const ItemNotesModal = ({ item, category, character, onClose, onSave }: { item: GameItem; category: ItemCategory; character: Character; onClose: () => void; onSave: (notes: any) => void; }) => {
  ...
  const handleSave = () => {
    ...
    currentNotes[category][item.id] = { 
        enhanced: isEnhanced, 
        bonus: isEnhanced ? bonusText : '',
        broken: isBroken
    };
      ...
      <div className="bg-[#fdfbf7] border-4 border-[#1a472a] rounded-lg max-w-lg w-full p-6 shadow-2xl flex flex-col animate-in zoom-in-95">
        ...
        <div className="space-y-6 font-serif text-stone-800">
          ...
          <div className={`p-4 border rounded-sm transition-colors ${isBroken ? 'bg-red-50 border-red-300' : 'bg-stone-50 border-stone-200'}`}>
             <h4 className="font-bold uppercase text-xs tracking-wider mb-3 flex items-center gap-2">
                <AlertTriangle size={14} className={isBroken ? 'text-red-600' : 'text-stone-400'}/>
                Condition & Durability
             </h4>
             <div className="flex items-center justify-between">
                <label htmlFor={uniqueId} className="text-sm font-bold text-stone-700 cursor-pointer select-none">
                    Is the item broken?
                    <span className="block text-[10px] font-normal text-stone-500 mt-0.5">
                        Happens if Parry Damage &gt; Durability ({item.durability || 'N/A'})
                    </span>
                </label>
                
                {/* TOGGLE SWITCH WRAPPER */}
                {/* Changed from <div> to <label> to make the whole graphic clickable */}
          ...
          <div className="space-y-4 bg-white p-4 border border-stone-200 rounded-sm">
            <div className="flex items-center gap-2 mb-2 text-[#1a472a]">
                <Hammer size={16} />
                <h4 className="font-bold uppercase text-xs tracking-wider">Mastercrafting</h4>
            </div>
            ...
            <div className="flex items-center">
              ...
              <label htmlFor="enhanced" className="ml-3 block text-sm font-bold text-stone-700 uppercase tracking-wide cursor-pointer">Mark as Enhanced (★)</label>
            </div>
...
export function EquipmentSection({ character }: EquipmentSectionProps) {
  ...
  // Helper to check broken status
  const isItemBroken = (item: GameItem | undefined, category: ItemCategory): boolean => getNoteForItem(item, category)?.broken || false;
  
  // Helper to check enhanced status
  ...
  const calculateTotalArmor = () => {
    ...
    const calc = (item: GameItem | undefined) => {
        ...
        if (note?.broken) return 0; // Broken armor provides 0 protection
        
        let val = Number(item.armor_rating) || 0;
        if (note?.enhanced) { 
            val += parseInt(note.bonus?.match(/\d+/)?.[0] || '0'); 
        }
        return val;
    };
  ...
  const handleDamageRoll = (weaponName: string, damageDiceString: string) => {
    ...
    const skillName = rawSkillName ? rawSkillName.toLowerCase() : null; 
          ...
          <div className="space-y-3">
            <div className="space-y-2">
              {[
                ...
                let bonusDisplay = '';
                if (item && note?.enhanced && note.bonus) {
                  ...
                  <div key={label} className={`flex flex-col gap-1 p-2 border rounded-sm transition-all ${isBroken ? 'bg-red-50 border-red-200 opacity-90' : 'bg-white border-stone-200'}`}>
                    <div className="flex justify-between items-center text-sm">
                      <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => item && setEditingItem({ item: item, category: 'armor' })} className={`font-serif font-bold text-left hover:underline disabled:cursor-default ${isBroken ? 'text-red-700 line-through decoration-2' : 'text-stone-800 hover:text-[#1a472a]'}`} disabled={!item}>
                    ...
                    {item && getBaneEffects(item) && !isBroken && (
                  ...
                  return (
                    <tr key={`${weapon.name}-${index}`} className={`group transition-colors ${isBroken ? 'bg-red-50 hover:bg-red-100' : 'hover:bg-[#f9f7f2]'}`}>
                      ...
                      <td className="px-3 py-2 text-center">
                          {isBroken ? (
                              <AlertTriangle size={14} className="text-red-500 mx-auto" title="Item is Broken"/>
                          ) : (
                              <span className="text-stone-600 text-xs">{weaponDetails?.durability || '-'}</span>
                          )}
                      </td>
                        ...
                        <div className="flex items-center justify-end gap-2 flex-wrap">
                          ...
                          {displayName && skillValue !== null && !isBroken && (
                            <button 
                              ...
                              title={`Attack or Parry with ${weapon.name}`}
