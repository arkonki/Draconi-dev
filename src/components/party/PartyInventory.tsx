      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl h-[85vh] flex flex-col overflow-hidden border border-gray-200 animate-in zoom-in-95 duration-200">
        ...
        <div className="flex-grow flex flex-col md:flex-row overflow-hidden">
                ...
                <div className="p-4 flex-grow overflow-y-auto space-y-4">
                   ...
                   <div className="grid grid-cols-2 gap-3">
                     <div>
                        ...
                        <input type="text" className="w-full p-2 border rounded text-sm uppercase" value={customForm.category} onChange={e => setCustomForm({...customForm, category: e.target.value})} placeholder="WEAPON, ARMOR, LOOT..."/>
                     </div>
                     <div>
                   ...
                   {isWeapon(customForm.category) && (
                      <div className="bg-red-50 p-3 rounded border border-red-100 space-y-3">
                         ...
                         <div className="grid grid-cols-2 gap-3">
                            <div><label className="block text-xs font-semibold text-red-700 mb-1">Damage</label><input type="text" className="w-full p-1.5 border border-red-200 rounded text-sm" value={customForm.damage} onChange={e => setCustomForm({...customForm, damage: e.target.value})} placeholder="e.g. 1D8"/></div>
                            <div><label className="block text-xs font-semibold text-red-700 mb-1">Grip</label><input type="text" className="w-full p-1.5 border border-red-200 rounded text-sm" value={customForm.grip} onChange={e => setCustomForm({...customForm, grip: e.target.value})} placeholder="1H, 2H"/></div>
                            <div><label className="block text-xs font-semibold text-red-700 mb-1">Range</label><input type="text" className="w-full p-1.5 border border-red-200 rounded text-sm" value={customForm.range} onChange={e => setCustomForm({...customForm, range: e.target.value})}/></div>
                            <div><label className="block text-xs font-semibold text-red-700 mb-1">Durability</label><input type="number" className="w-full p-1.5 border border-red-200 rounded text-sm" value={customForm.durability} onChange={e => setCustomForm({...customForm, durability: e.target.value})}/></div>
                         </div>
                         <div><label className="block text-xs font-semibold text-red-700 mb-1">Features</label><input type="text" className="w-full p-1.5 border border-red-200 rounded text-sm" value={customForm.features} onChange={e => setCustomForm({...customForm, features: e.target.value})} placeholder="Piercing, Subtle..."/></div>
                      </div>
                   )}
                   ...
                   {isArmor(customForm.category) && (
                      <div className="bg-blue-50 p-3 rounded border border-blue-100 space-y-3">
                         ...
                         <div><label className="block text-xs font-semibold text-blue-700 mb-1">Armor Rating</label><input type="number" className="w-full p-1.5 border border-blue-200 rounded text-sm" value={customForm.armor_rating} onChange={e => setCustomForm({...customForm, armor_rating: e.target.value})} placeholder="e.g. 2"/></div>
                      </div>
                   )}

                   <div className="grid grid-cols-2 gap-3">
                     <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Weight</label><input type="number" step="0.1" className="w-full p-2 border rounded text-sm" value={customForm.weight} onChange={e => setCustomForm({...customForm, weight: e.target.value})} /></div>
                     <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Quantity</label><input type="number" min="1" max="50" className="w-full p-2 border rounded text-sm" value={customForm.quantity} onChange={e => setCustomForm({...customForm, quantity: parseInt(e.target.value)})} /></div>
                   </div>

                   <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Effect / Description</label><textarea className="w-full p-2 border rounded text-sm h-20 resize-none" value={customForm.effect} onChange={e => setCustomForm({...customForm, effect: e.target.value})} /></div>
                </div>
              ...
              <>
                <div className="p-4 border-b space-y-3">
                  <div className="relative">
                    ...
                    <input autoFocus type="text" placeholder="Search database..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-9 pr-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
                  </div>
                  <Button variant="secondary" size="sm" icon={Plus} className="w-full justify-center" onClick={() => handleStartCustomizing()}>Create Custom Item</Button>
                </div>
                <div className="flex-grow overflow-y-auto p-2 space-y-1 bg-gray-50/50">
                  {filteredItems.length > 0 ? (
                    filteredItems.map(item => (
                      <div key={item.id} className="p-3 bg-white border border-gray-200 rounded-lg shadow-sm flex items-center gap-3 hover:border-blue-300 transition-colors group relative">
                        ...
                        <div className="flex items-center gap-2">
                           <button onClick={() => handleStartCustomizing(item)} className="p-1.5 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded" title="Copy & Customize"><Pencil size={16} /></button>
                           <form className="flex items-center gap-1" onSubmit={(e) => { e.preventDefault(); const qInput = e.currentTarget.elements.namedItem('qty') as HTMLInputElement; handleStageItem(item, parseInt(qInput.value) || 1); qInput.value = '1'; }}>
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-gray-400 p-8 text-center"><Search size={32} className="mb-2 opacity-20"/><p className="text-sm">Search or create custom item.</p></div>
                  )}
                </div>
              </>
            )}
          ...
          <div className="w-full md:w-1/2 flex flex-col bg-gray-50">
            <div className="p-3 bg-blue-50 border-b border-blue-100 flex justify-between items-center"><h4 className="font-bold text-blue-900 text-sm uppercase tracking-wide">Pending Loot ({stagedLoot.size})</h4></div>
            <div className="flex-grow overflow-y-auto p-3 space-y-2">
              {stagedLoot.size === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-gray-400 border-2 border-dashed border-gray-200 rounded-lg m-4"><Package size={32} className="mb-2 opacity-20"/><p className="text-sm">No items added yet.</p></div>
              ) : (
                  ...
                  <div key={item.id} className="flex items-center justify-between p-3 bg-white border rounded-lg shadow-sm animate-in slide-in-from-left-2">
                    <div className="flex items-center gap-3">
                       ...
                       <div className="min-w-0">
                         <p className="font-medium text-sm text-gray-900 truncate">{item.name}</p>
                         <div className="flex gap-2 text-xs text-gray-500"><span className="uppercase">{item.category}</span>{item.id.startsWith('custom-') && <span className="text-amber-600 flex items-center gap-0.5"><AlertCircle size={10}/> Custom</span>}</div>
                       </div>
                    </div>
        ...
        <div className="p-4 border-t bg-white flex justify-end gap-3">
          ...
          <Button variant="primary" onClick={handleConfirmLoot} disabled={stagedLoot.size === 0 || isProcessing} icon={Plus}>{isProcessing ? 'Adding...' : `Add Items to Party`}</Button>
        </div>
      </div>
    </div>
  );
...
export function PartyInventory({ partyId, members, isDM }: PartyInventoryProps) {
  ...
  useEffect(() => {
    ...
    // Subscribe to REALTIME changes for this party's inventory
    const subscription = supabase
      .channel(`party_inv_${partyId}`)
      .on(
        'postgres_changes',
        {
        ...
        () => {
          // When any change happens, reload the data to stay in sync
          loadData();
        }
      )
      .subscribe();
  ...
  async function loadData() {
    // Note: Don't set global loading=true here to prevent flickering on realtime updates
    try {
    ...
    try {
      ...
      for (const { item, quantity } of loot) {
        ...
        if (existing) {
          ...
          await supabase.from('party_inventory').insert([{ party_id: partyId, name: item.name, quantity, category: item.category, description }]);
        }
      ...
      // loadData() will be triggered by realtime subscription, but we call it for instant local feedback if desired
      await loadData();
    } catch (err) {
    ...
    try {
      for (const itemId of selectedItemIds) {
        ...
        if (existingIdx >= 0) {
          ...
          newInv.push({ name: partyItem.name, quantity: 1, category: partyItem.category || details?.category || 'LOOT', description: partyItem.description || details?.effect });
        }
  ...
  const handleTransferToParty = async (charItem: CharacterInventoryItem) => {
    ...
    try {
      ...
      if (existingPartyItem) {
        ...
        await supabase.from('party_inventory').insert([{ party_id: partyId, name: charItem.name, quantity: 1, category: charItem.category || details?.category || 'LOOT', description: charItem.description || details?.effect || details?.description }]);
      }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-[calc(100vh-140px)]">
        <div className="lg:col-span-7 flex flex-col bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-4 border-b border-gray-100 flex flex-wrap justify-between items-center gap-4 bg-white z-10">
            <div className="flex items-center gap-2">
               ...
               <div><h2 className="text-lg font-bold text-gray-900">Party Stash</h2><p className="text-xs text-gray-500">{inventory.reduce((acc, i) => acc + i.quantity, 0)} Items total</p></div>
            </div>
        ...
        <div className="lg:col-span-5 flex flex-col gap-4 h-full">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 flex-shrink-0">
            ...
            <select className="w-full p-2.5 border border-gray-300 rounded-lg text-sm font-medium focus:ring-2 focus:ring-indigo-500 outline-none" value={selectedCharacterId} onChange={e => setSelectedCharacterId(e.target.value)}>
               ...
               <div className="flex-1 overflow-y-auto p-4 space-y-3">
                 {transactionLog.map(log => (
                   <div key={log.id} className="text-xs p-2 bg-gray-50 rounded border border-gray-100">
                      ...
                      <div className="flex items-center gap-1 text-gray-500 mt-1"><span>{log.from_type === 'party' ? 'Party' : 'Char'}</span><ArrowRight size={10}/><span>{log.to_type === 'party' ? 'Party' : 'Char'}</span></div>
