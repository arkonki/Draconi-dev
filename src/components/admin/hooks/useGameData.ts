...
import { MonsterData } from '../../../types/bestiary';
...
setLoading(true);
  ...
  if (category === 'monsters') {
    // Ensure stats and attacks are correctly formatted if they exist
    if (dataToSave.stats && typeof dataToSave.stats !== 'object') {
        dataToSave.stats = {}; // Default to empty object if invalid
    }
    if (dataToSave.attacks && !Array.isArray(dataToSave.attacks)) {
        dataToSave.attacks = []; // Default to empty array if invalid
    }
  }
  if (category === 'bio') {
    // Ensure arrays are clean of empty strings before saving
    dataToSave.appearance = (dataToSave.appearance || []).filter(Boolean);
    dataToSave.mementos = (dataToSave.mementos || []).filter(Boolean);
    dataToSave.flaws = (dataToSave.flaws || []).filter(Boolean);
  }
  // --- End Transformations ---
